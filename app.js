...
var bot = linebot({
    channelId: process.env.BOT_CHANNELID,
    channelSecret: process.env.BOT_CHANNELSECRET,
...
function helpBoss(event, clans) {
    ...
    message += "目前開放的指令有：boss(時間表), note(備註), alert(提醒), add(加入), remove(刪除), set(設定), clear(清除)\n\n";
    message += "指令輸入中文或英文皆可。\n\n";
    
    message += "若使用上有任何問題請聯繫：\n";
    message += "LineID：daje770729\n";
    message += "Mail：daje770729@gmail.com\n\n";
    ...
    message += "指令： alert [分鐘]\n";
    message += "範例： alert 15\n";
    message += "這樣代表每隻王重生前15分鐘會先提醒，設定為 0 代表關閉提醒功能。\n\n";
